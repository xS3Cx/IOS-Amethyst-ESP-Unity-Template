THEOS_DEVICE_IP = 192.168.0.100 # install to device from pc
ARCHS = arm64 #arm64e
DEBUG = 0
FINALPACKAGE = 1
FOR_RELEASE = 1
...
Amethyst_CCFLAGS = -std=c++11 -fno-rtti -fno-exceptions -DNDEBUG
...
Amethyst_FILES = Amethyst.xm $(MENU_SRC) $(KITTYMEMORY_SRC) $(SCLALERTVIEW_SRC) $(wildcard *.m) $(wildcard *.mm) $(wildcard */*.m) $(wildcard */*.mm) $(wildcard */*.cpp)

Amethyst_LIBRARIES += substrate
Amethyst_OBJ_FILES = DobbyHook/libdobby.a
Amethyst_FRAMEWORKS = $(PROJ_COMMON_FRAMEWORKS)
# GO_EASY_ON_ME = 1
